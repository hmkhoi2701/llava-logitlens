...
def process_images(image_folder, save_folder, device, quantize_type, num_images):
    ...
    for image_path, image in tqdm(images.items()):
        ...
        str_output = model.generate(image, prompt, max_new_tokens=500)
        hidden_states = model.forward(image, prompt, output_hidden_states=True).hidden_states
        create_interactive_logit_lens(hidden_states, norm, lm_head, tokenizer, image, model_name, image_path, prompt, str_output, save_folder)

def main():
    parser = argparse.ArgumentParser(description="Process images using HookedLVLM model")
    parser.add_argument("--image_folder", default="data/images", help="Path to the folder containing images")
    parser.add_argument("--save_folder", default="output", help="Path to save the results")
